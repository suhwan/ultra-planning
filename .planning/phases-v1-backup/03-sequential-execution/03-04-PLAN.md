...
files_modified:
  - .claude/skills/ultraplan/references/verification-gate.md
  - .claude/skills/ultraplan/references/state-protocol.md
autonomous: true

must_haves:
  truths:
    - "Every task passes through Execute -> Verify Command -> Architect Review gate"
    - "Task cannot be marked complete without verification command exit code 0"
    - "Architect approval is required after verification command passes"
    - "Gate failure modes are documented with recovery paths"
    - "STATE.md reflects gate status (executing/verifying/approved/failed)"
  artifacts:
    - path: ".claude/skills/ultraplan/references/verification-gate.md"
      provides: "Complete verification gate protocol documentation"
      contains: "VERIFICATION GATE FLOW"
      min_lines: 200
  key_links:
    - from: ".claude/skills/ultraplan/references/verification-gate.md"
...
<objective>
Define the verification gate protocol that every task must pass before being marked complete.

Purpose: The verification gate is the quality control checkpoint. It ensures no unverified work is committed to the codebase, preventing broken builds and incomplete implementations. Every task follows the same path: Execute -> Verify Command -> Architect Review.

Output: `.claude/skills/ultraplan/references/verification-gate.md` with complete gate protocol
</objective>
...
<task type="auto">
  <name>Task 1: Create Verification Gate Reference with Core Flow</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md</files>
  <action>
Create the verification gate reference document with:
...
```
  ...
  <verify>
cat .claude/skills/ultraplan/references/verification-gate.md | head -80
grep -c "VERIFICATION GATE FLOW" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Step 1: Executor Output" .claude/skills/ultraplan/references/verification-gate.md
  </verify>
  <done>Verification gate reference created with core flow diagram and step-by-step protocol</done>
</task>

<task type="auto">
  <name>Task 2: Define "Passing" Criteria and Evidence Requirements</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md</files>
  <action>
Add the "passing" definition section:
...
```
  ...
  <verify>
grep -c "What \"Passing\" Means" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Evidence Chain" .claude/skills/ultraplan/references/verification-gate.md
grep -c "What is NOT a Pass" .claude/skills/ultraplan/references/verification-gate.md
  </verify>
  <done>Passing criteria defined with evidence requirements and examples</done>
</task>

<task type="auto">
  <name>Task 3: Document Gate Failure Modes</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md</files>
  <action>
Add the failure modes section:
...
```
  ...
  <verify>
grep -c "Gate Failure Modes" .claude/skills/ultraplan/references/verification-gate.md
grep -c "EXECUTION_FAILURE" .claude/skills/ultraplan/references/verification-gate.md
grep -c "ARCHITECT_REJECTION" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Recovery Path" .claude/skills/ultraplan/references/verification-gate.md
  </verify>
  <done>Gate failure modes documented with triggers, evidence, and recovery paths</done>
</task>

<task type="auto">
  <name>Task 4: Add Escalation Rules and Manual Override</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md</files>
  <action>
Add escalation rules and override mechanisms:
...
```
  ...
  <verify>
grep -c "Escalation Rules" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Manual Override" .claude/skills/ultraplan/references/verification-gate.md
grep -c "skip-verify" .claude/skills/ultraplan/references/verification-gate.md
grep -c "force-complete" .claude/skills/ultraplan/references/verification-gate.md
  </verify>
  <done>Escalation rules and manual override mechanisms documented</done>
</task>

<task type="auto">
  <name>Task 5: Add Gate Status Indicators for STATE.md</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md, .claude/skills/ultraplan/references/state-protocol.md</files>
  <action>
Add gate status indicators:

1. In verification-gate.md, add:
```markdown
...
2. Update state-protocol.md to include gate status:
```markdown
## Gate Status Updates
...
```
  ...
  <verify>
grep -c "Gate Status Indicators" .claude/skills/ultraplan/references/verification-gate.md
grep -c "gate_status" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Visual Status Bar" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Gate Status Updates" .claude/skills/ultraplan/references/state-protocol.md
  </verify>
  <done>Gate status indicators added for STATE.md tracking</done>
</task>

<task type="auto">
  <name>Task 6: Add Integration Summary and Best Practices</name>
  <files>.claude/skills/ultraplan/references/verification-gate.md</files>
  <action>
Add integration summary and best practices:
...
```
  ...
  <verify>
grep -c "Integration with Other Components" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Best Practices" .claude/skills/ultraplan/references/verification-gate.md
grep -c "Quick Reference Table" .claude/skills/ultraplan/references/verification-gate.md
wc -l .claude/skills/ultraplan/references/verification-gate.md
  </verify>
  <done>Integration summary and best practices added; document complete with 200+ lines</done>
</task>

</tasks>

<verification>
1. File exists: `ls -la .claude/skills/ultraplan/references/verification-gate.md`
2. Core flow documented: `grep "VERIFICATION GATE FLOW" .claude/skills/ultraplan/references/verification-gate.md`
3. Passing criteria defined: `grep "What \"Passing\" Means" .claude/skills/ultraplan/references/verification-gate.md`
4. Failure modes documented: `grep "Gate Failure Modes" .claude/skills/ultraplan/references/verification-gate.md`
5. Escalation rules present: `grep "Escalation Rules" .claude/skills/ultraplan/references/verification-gate.md`
6. Manual override documented: `grep "Manual Override" .claude/skills/ultraplan/references/verification-gate.md`
7. Status indicators defined: `grep "Gate Status Indicators" .claude/skills/ultraplan/references/verification-gate.md`
8. State protocol updated: `grep "Gate Status Updates" .claude/skills/ultraplan/references/state-protocol.md`
9. Minimum length: `wc -l .claude/skills/ultraplan/references/verification-gate.md` (should be >= 200)

**Structural Validation:** After grep checks pass, visually confirm:
- Three-step flow clearly documented (Executor -> Verify -> Architect)
- Pass/fail criteria are unambiguous
- All failure modes have recovery paths
- Override mechanisms require explicit user action
- STATE.md integration is complete
</verification>

<success_criteria>
- [ ] Verification gate reference file created
- [ ] Three-step flow documented (Executor -> Verify Command -> Architect Review)
- [ ] "Passing" definition is clear with evidence requirements
- [ ] All failure modes documented with recovery paths
- [ ] Escalation rules defined with thresholds
- [ ] Manual override requires explicit user action and is logged
- [ ] Gate status indicators defined for STATE.md
- [ ] State protocol updated with gate status events
- [ ] Best practices for verify commands included
- [ ] At least 200 lines of documentation
</success_criteria>

<output>
After completion, create `.planning/phases/03-sequential-execution/03-04-SUMMARY.md`
</output>
