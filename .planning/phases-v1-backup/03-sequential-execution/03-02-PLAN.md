...
must_haves:
  truths:
    - "Spawn protocol document exists with complete orchestrator patterns"
    - "Prompt template injects task XML into fresh executor context"
    - "Each spawned agent receives full 200k context window"
    - "Result parsing handles success, failure, and blocked states"
    - "Error handling covers spawn failures, timeouts, and malformed responses"
  artifacts:
    - path: ".claude/skills/ultraplan/references/spawn-protocol.md"
      provides: "Complete spawning protocol for fresh executor agents"
      contains: "Task("
      min_lines: 200
  key_links:
    - from: ".claude/skills/ultraplan/references/spawn-protocol.md"
      to: ".claude/agents/ultraplan-executor.md"
      via: "Task tool spawning with explicit model parameter"
      pattern: "subagent_type=\"oh-my-claudecode:ultraplan-executor\""
---

<objective>
Create the spawning protocol document that defines HOW the orchestrator spawns executor agents with fresh context.

Purpose: The orchestrator (router) needs a well-defined protocol for spawning executor subagents. Each task from PLAN.md must be injected into a fresh 200k context window, executed, and results parsed back. This document serves as the authoritative reference for spawn mechanics.

Output: `.claude/skills/ultraplan/references/spawn-protocol.md` with complete spawning patterns
</objective>
...
<task type="auto">
  <name>Task 1: Create Spawn Protocol Document with Core Concepts</name>
  <files>.claude/skills/ultraplan/references/spawn-protocol.md</files>
  <action>
Create the spawn protocol reference document with header and core concepts:
...
```
  ...
  <verify>
cat .claude/skills/ultraplan/references/spawn-protocol.md | head -50
grep -c "Fresh Context" .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Document exists with core concepts (fresh context, spawn vs direct, model selection)</done>
</task>

<task type="auto">
  <name>Task 2: Add Task Tool Spawning Syntax Section</name>
  <files>.claude/skills/ultraplan/references/spawn-protocol.md</files>
  <action>
Add the Task tool spawning syntax section:
...
```
  ...
  <verify>
grep -c "subagent_type" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "model=" .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Task tool spawning syntax documented with parameter reference and model requirement</done>
</task>
...
```
  ...
  <verify>
grep -c "Prompt Template" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Task Execution Request" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "{raw_task_xml}" .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Prompt template section complete with variables and example injection</done>
</task>

<task type="auto">
  <name>Task 4: Add Result Parsing Logic Description</name>
  <files>.claude/skills/ultraplan/references/spawn-protocol.md</files>
  <action>
Add the result parsing section:
...
```
  ...
  <verify>
grep -c "Result Parsing" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Parsing Steps" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Parse Failure" .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Result parsing section complete with validation rules and failure handling</done>
</task>

<task type="auto">
  <name>Task 5: Add Error Handling for Spawn Failures</name>
  <files>.claude/skills/ultraplan/references/spawn-protocol.md</files>
  <action>
Add the error handling section:
...
```
  ...
  <verify>
grep -c "Error Handling" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Spawn Failure Protocol" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Model Fallback" .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Error handling section complete with categories, protocols, and recovery strategies</done>
</task>

<task type="auto">
  <name>Task 6: Add Complete Spawn Flow Example</name>
  <files>.claude/skills/ultraplan/references/spawn-protocol.md</files>
  <action>
Add a complete example showing end-to-end spawn flow:
...
```
  ...
  <verify>
grep -c "Complete Spawn Flow" .claude/skills/ultraplan/references/spawn-protocol.md
grep -c "Spawn Protocol Checklist" .claude/skills/ultraplan/references/spawn-protocol.md
wc -l .claude/skills/ultraplan/references/spawn-protocol.md
  </verify>
  <done>Complete spawn flow example added with step-by-step walkthrough and checklist</done>
</task>

</tasks>

<verification>
1. File exists: `ls -la .claude/skills/ultraplan/references/spawn-protocol.md`
2. Has core concepts: `grep "Fresh Context" .claude/skills/ultraplan/references/spawn-protocol.md`
3. Has Task tool syntax: `grep "subagent_type" .claude/skills/ultraplan/references/spawn-protocol.md`
4. Has prompt template: `grep "Prompt Template" .claude/skills/ultraplan/references/spawn-protocol.md`
5. Has result parsing: `grep "Result Parsing" .claude/skills/ultraplan/references/spawn-protocol.md`
6. Has error handling: `grep "Error Handling" .claude/skills/ultraplan/references/spawn-protocol.md`
7. Has complete example: `grep "Complete Spawn Flow" .claude/skills/ultraplan/references/spawn-protocol.md`
8. Minimum length: `wc -l .claude/skills/ultraplan/references/spawn-protocol.md` (should be >= 200 lines)

**Structural Validation:** After grep checks pass, visually confirm:
- Core concepts explain fresh context advantage and model selection
- Task tool syntax shows explicit model parameter requirement
- Prompt template has all variables documented
- Result parsing handles success, failure, and blocked
- Error handling covers spawn failures, timeouts, parse errors
- Complete example shows end-to-end flow
</verification>

<success_criteria>
- [ ] Spawn protocol document created at `.claude/skills/ultraplan/references/spawn-protocol.md`
- [ ] Core concepts section explains fresh 200k context advantage
- [ ] Task tool syntax requires explicit model parameter
- [ ] Prompt template documented with all injection variables
- [ ] Result parsing handles all three status types (success, failure, blocked)
- [ ] Error handling covers spawn failures, timeouts, model fallback
- [ ] Complete example shows orchestrator perspective from extract to update
- [ ] At least 200 lines of structured protocol content
</success_criteria>

<output>
After completion, create `.planning/phases/03-sequential-execution/03-02-SUMMARY.md`
</output>
