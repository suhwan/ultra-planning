...
files_modified:
  - .claude/skills/ultraplan/references/retry-protocol.md
autonomous: true

must_haves:
  truths:
    - "Failed task verification triggers retry loop"
    - "Maximum 3 retries per task before escalation"
    - "Each retry includes previous failure feedback in executor prompt"
    - "Feedback injection follows structured format"
    - "After 3 failures, escalation to user with full context"
    - "Retry attempts are logged with timestamps and failure reasons"
  artifacts:
    - path: ".claude/skills/ultraplan/references/retry-protocol.md"
      provides: "Complete retry loop specification with feedback injection format"
      contains: "RETRY LOOP PROTOCOL"
      min_lines: 180
  key_links:
    - from: ".claude/skills/ultraplan/references/retry-protocol.md"
      to: ".claude/agents/ultraplan-executor.md"
      via: "Feedback injection into executor prompt"
      pattern: "RETRY CONTEXT"
    - from: ".claude/skills/ultraplan/references/retry-protocol.md"
      to: ".claude/skills/ultraplan/references/state-protocol.md"
      via: "Retry state tracking and logging"
      pattern: "retry_count"
---

<objective>
Define the retry loop protocol for failed task verification with structured feedback injection.

Purpose: When an executor fails a task or verification fails, the system needs a clear protocol for retrying with accumulated failure context. This prevents repeated failures from the same root cause by injecting previous failure feedback into the retry attempt, while escalating to human intervention when automated recovery is impossible.

Output: `.claude/skills/ultraplan/references/retry-protocol.md` with complete retry specification
</objective>
...
<task type="auto">
  <name>Task 1: Create Retry Protocol Core with Loop Flow</name>
  <files>.claude/skills/ultraplan/references/retry-protocol.md</files>
  <action>
Create the retry protocol reference document with:

1. Document header and overview:
```markdown
# Retry Loop Protocol
...
```
  ...
  <verify>
grep -c "RETRY LOOP PROTOCOL" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "MAX_RETRIES" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "Retry Triggers" .claude/skills/ultraplan/references/retry-protocol.md
  </verify>
  <done>Retry protocol document created with loop flow and trigger conditions</done>
</task>

<task type="auto">
  <name>Task 2: Add Feedback Injection Format</name>
  <files>.claude/skills/ultraplan/references/retry-protocol.md</files>
  <action>
Add the feedback injection specification:

1. Feedback capture format:
```markdown
...
```
  ...
  <verify>
grep -c "RETRY CONTEXT" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "Feedback Injection Format" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "previous_failures" .claude/skills/ultraplan/references/retry-protocol.md
  </verify>
  <done>Feedback injection format documented with retry_context XML block and placement rules</done>
</task>

<task type="auto">
  <name>Task 3: Add Retry Counter Tracking</name>
  <files>.claude/skills/ultraplan/references/retry-protocol.md</files>
  <action>
Add retry counter tracking specification:
...
```
  ...
  <verify>
grep -c "retry_count" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "retry-state.json" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "State Transitions" .claude/skills/ultraplan/references/retry-protocol.md
  </verify>
  <done>Retry counter tracking documented with state file schema and state transitions</done>
</task>

<task type="auto">
  <name>Task 4: Add Escalation Protocol</name>
  <files>.claude/skills/ultraplan/references/retry-protocol.md</files>
  <action>
Add escalation protocol for user intervention:
...
```
  ...
  <verify>
grep -c "Escalation Protocol" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "Escalation Message Format" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "User Response" .claude/skills/ultraplan/references/retry-protocol.md
  </verify>
  <done>Escalation protocol documented with message format and user response handling</done>
</task>

<task type="auto">
  <name>Task 5: Add Retry Logging for Debugging</name>
  <files>.claude/skills/ultraplan/references/retry-protocol.md</files>
  <action>
Add retry logging specification:
...
```
  ...
  <verify>
grep -c "Retry Logging" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "retry.log" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "retry.jsonl" .claude/skills/ultraplan/references/retry-protocol.md
grep -c "Retry Summary" .claude/skills/ultraplan/references/retry-protocol.md
wc -l .claude/skills/ultraplan/references/retry-protocol.md
  </verify>
  <done>Retry logging documented with text/JSON formats, retention policy, and summary statistics</done>
</task>

</tasks>

<verification>
1. File exists: `ls -la .claude/skills/ultraplan/references/retry-protocol.md`
2. Has loop protocol: `grep "RETRY LOOP PROTOCOL" .claude/skills/ultraplan/references/retry-protocol.md`
3. Has feedback injection: `grep "RETRY CONTEXT" .claude/skills/ultraplan/references/retry-protocol.md`
4. Has counter tracking: `grep "retry_count" .claude/skills/ultraplan/references/retry-protocol.md`
5. Has escalation: `grep "Escalation Protocol" .claude/skills/ultraplan/references/retry-protocol.md`
6. Has logging: `grep "retry.log" .claude/skills/ultraplan/references/retry-protocol.md`
7. Minimum length: `wc -l .claude/skills/ultraplan/references/retry-protocol.md` (should be >= 180 lines)

**Structural Validation:** After grep checks pass, visually confirm:
- Loop flow diagram is ASCII-renderable
- Feedback XML format is valid and injectable
- State schema matches state-protocol.md patterns
- Escalation message is user-friendly
- Log formats are parseable
</verification>

<success_criteria>
- [ ] Reference document created at `.claude/skills/ultraplan/references/retry-protocol.md`
- [ ] Retry loop flow documented with diagram
- [ ] Retry trigger conditions defined (when to retry vs escalate)
- [ ] Feedback injection format specified with `<retry_context>` XML block
- [ ] Feedback placement rule: BEFORE task XML in prompt
- [ ] Retry counter tracking with state file schema
- [ ] State transitions documented (pending -> executing -> done/retrying/escalated)
- [ ] Escalation protocol with user message format
- [ ] User response handling (retry, skip, abort, fix)
- [ ] Skip handling with dependency impact warning
- [ ] Retry logging to .ultraplan/logs/retry.log
- [ ] Structured JSON log to .ultraplan/logs/retry.jsonl
- [ ] Debugging commands documented
- [ ] Retry statistics summary format
- [ ] At least 180 lines of specification content
</success_criteria>

<output>
After completion, create `.planning/phases/03-sequential-execution/03-06-SUMMARY.md`
</output>
