 ...
 */
export async function saveQualityResult(result: QualityCheckResult): Promise<void> {
  const state = new StateManager();

  // Load existing results
  const existing = await loadQualityResult();
  ...
  await state.write(QUALITY_STATE_NAME, results);
}

/**
 * Load quality results from state
 */
export async function loadQualityResult(): Promise<{ checks: QualityCheckResult[] } | undefined> {
  const state = new StateManager();
  const result = await state.read<{ checks: QualityCheckResult[] }>(QUALITY_STATE_NAME);
  return result.data;
}
 ...
 */
export async function getLatestQualityResult(): Promise<QualityCheckResult | undefined> {
  const results = await loadQualityResult();
 ...
 */
export async function getQualityResultForTask(taskId: string): Promise<QualityCheckResult | undefined> {
  const results = await loadQualityResult();
...
```
  ...
  <verify>npm run build; Test pipeline: `npx ts-node -e "import {runQualityPipeline} from './src/quality/pipeline/index.js'; runQualityPipeline().then(r => console.log(JSON.stringify(r.summary, null, 2)))"`</verify>
