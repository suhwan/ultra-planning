...
must_haves:
  truths:
    - "User can run /ultraplan:new-project to start planning"
    - "Command invokes the ultraplan-planner agent"
    - "Command provides initial context to planner"
  artifacts:
...
2. **Spawn Planner Agent**
   - Use Task tool to invoke `ultraplan-planner` agent
   - Pass initial context:
     - User's project description (if provided)
     - Current directory name as default project name
     - Existing codebase summary (if brownfield)

3. **Planner Takes Over**
   - Planner conducts interview
   - Planner generates documents
   - Planner requests review
...
Project hint: {user_description_or_directory_name}
Environment: {current_directory}
Existing code: {yes/no, with summary if yes}

Begin with interview. Ask ONE question at a time.
```
